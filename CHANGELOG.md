...
- [#282](https://github.com/chaitanyagiri/munder-difflin/pull/282) [@savvaskoualis](https://github.com/savvaskoualis): a renamed boss identity no longer reverts to "Hakan"
...
- **A hero card at the top of Settings** with your version, your plan, and a way back to the
  release notes.
- **A 1:1 hold** that tells Hakan to leave an agent alone.
- **Editable agent names,** and Hakan can choose a worker's avatar and accent when he hires.
...
- Agent selection is visible on every card including Hakan's — it used to be drawn in each
...
- **Prerequisites** (Settings) — live status for uv, git, Node, MemPalace and every agent engine,
  with real paths, platform-correct install commands, and a button that asks Hakan to fill the
  gaps.
...
- One card size for every agent; Hakan is distinguished by surface, not by a heavier border.
...
**A new brand mark: Hakan's portrait replaces the "MD" tile.**
...
### Changed
- **Hakan is your clone.** Onboarding refers to him as your clone throughout, and his card on
...
- **Compact Command Center header.** At sidebar width the old header wrapped its
  ...
  CENTER** title + "Hakan runs the floor" subtitle (both ellipsize), and the floor
...
**The queue you can trust, a Hakan who actually knows the floor, and an IDE that shows
...
first-party features — **voice Hakan with live floor context + full app control**,
...
- **Talk mode grows up: live context + full control.** Hakan's voice session now opens
...
- **Remote Control sessions are named after the agent** ([#81](https://github.com/chaitanyagiri/munder-difflin/pull/81)). claude.ai / the mobile app now shows "Hakan", "Caner", … instead of `<hostname>-<random>`, so a floor full of RC sessions is finally tellable-apart. *(gts-47)*
...
**Talk to Hakan.** The headline is **Realtime Hakan** — a low-latency **voice channel to the
BOSS orchestrator**, running alongside the async terminal floor. Press **Talk**, and Hakan listens,
...
> OpenAI key — connect → mic → Hakan answers via the read tools, and the full destructive action
...
### Added
- **Realtime Hakan — talk to the BOSS orchestrator by voice.** A new low-latency realtime channel
  (OpenAI Realtime API over WebRTC) sits next to the async terminal. A **Talk** toggle (on Hakan's
  card and in any fullscreen terminal) opens a mic session with EC/NS/AGC, semantic-VAD turn-taking +
  barge-in, and a device picker for both microphone and speaker. Hakan runs his own persona and
...
- **Voice action set with tiered echo-back confirmation.** Hakan can *do* things by voice — read
...
- **"Respond when done" completion loop.** Voice-dispatched work reports back on its own: a
  ...
  reply in the inbox) and **proactively pushes the event into the live session so Hakan speaks it
...
- **Greeting on connect.** When a session goes live, Hakan opens with a warm, rotating greeting
...
- **Conversational read-layer.** The voice read tools were reworked to actually answer hive
  ...
  Hakan talk through roster, tasks, and floor state naturally.
- **Voice read-layer over hive messages (read/brief-only).** Realtime Hakan can now read message
...
- **Talk reachable from any fullscreen terminal.** The Talk toggle is no longer Hakan-only chrome —
  it's reachable in any fullscreen terminal view (the toggle is global/session state, so it's correct
  everywhere), while the per-session cost HUD stays Hakan-only (`FullscreenTerminal.tsx`).
...
worker *and* as Hakan, with **bring-your-own keys + local LLMs**. Plus two reliability fixes: the
...
every hire — and Hakan himself — a pluggable engine (Claude Code / Antigravity / Codex /
...
write-only, registry-driven Settings flow. Hakan can now **spawn an ephemeral worker straight
...
- **Swappable Hakan (boss) engine.** The orchestrator is no longer hard-wired to one CLI: `useHive` gains an engine-spawn path, Onboarding gains an **engine picker** for Hakan, and a **change-engine flow** lets you re-home the boss orchestrator onto a different engine without rebuilding the floor.
...
### Fixed
- **Orchestrator delegates opportunistically to existing agents.** Hakan now checks the live roster (active agents in `registry.json` + their state in `fleet.json`) before spawning, and prefers routing a task to an existing agent that already fits — above all when the request names one ("ask Selin…", "have Caner…") — instead of reflexively creating a new agent; he only spawns a fresh one when no existing agent is a sensible fit, and says that he checked. Encoded in both the floor orchestrator prompt and the Slack autonomous-request protocol (`src/main/hive.ts`, `src/main/index.ts`).
...
- **The Hiring Fair — community gallery** at [munderdiffl.in/hires](https://munderdiffl.in/hires/) (`docs/hires/`, static, no build step, served by the existing GitHub Pages setup). Seed roles drawn from the cast (Selin writes docs, Batur enforces QA, Caner reviews PRs, Ertan audits security, Deniz audits the office's own token spend, Polat does the migrations nobody wants), each with a Claude Code / Antigravity / Codex provider toggle (per-provider variants generated from one base manifest), function filters matching the landing page, and a client-side validator identical to the app's alongside a JSON schema (`docs/hires/spec/`). Model suggestions are data-driven (`docs/hires/models.json`), so new models are a one-line update.
...
- **All human dispatch flows through the boss.** Every Command Center dispatch now mails the boss (`Task from the human`) instead of writing straight into a worker's inbox; the worker dropdown becomes a **suggested owner** (Hakan still decides), so nothing skips the orchestrator. (Thanks @Gulum — #45, fixes #44.)
...
- **Heartbeat is inbox-driven.** The floor heartbeat (`reengageBoss`) no longer types directly into Hakan's terminal; it drops its digest in his inbox, which the busy-aware inbox-wake delivers once he's idle.

### Docs
- Expanded the README roadmap (chat integrations, pluggable agent CLIs, realtime Hakan).
...
### Added
- **Command Center overhaul.** Hakan's control surface was reworked into the place you actually run the floor from — the roster, dispatch, schedules, memory, and activity views now carry the new live signals (token budgets, telemetry, breaker state) without becoming a wall of numbers.
...
### Added
- **Batur, Hakan's prep assistant.** A persistent, visible assistant agent
  (Sonnet, 1M context) spawns on startup. A global **enrich** toggle routes
  Hakan's queued prompts through Batur first — he gathers repo context and
  rewrites the prompt, then forwards it to Hakan through the hive; toggle it off
  and prompts go straight to Hakan.
- **Hakan's Command Center.** His sidebar becomes a control surface with
...
- **Settings panel** (title-bar gear) with a **Reset & start over** action that wipes
  Hakan's memories, the entire hive (every agent, message, task, and the board), and
...
- Hakan no longer marches to the door flagged "needs you" right after finishing a turn —
