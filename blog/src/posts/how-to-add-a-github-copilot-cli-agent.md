...
You need Munder Difflin itself first — if you don't, [the install guide](/blog/how-to-install-and-use-munder-difflin/) covers it in a few minutes. Everything below assumes you're looking at the office floor with Hakan seated in his office.
...
Copilot hires also work through voice — Realtime Hakan's `spawn` action can hire one — and if you paste a raw `copilot ...` command into the custom-command field, the harness infers the binary and treats it as a Copilot engine.
...
So what happens when another agent routes a message to your Copilot worker? It **bounces to the BOSS orchestrator** instead of silently dropping. Hakan sees the bounced mail and handles or re-routes it — the message survives, it just doesn't land in a mailbox nobody is checking. (If you're new to how Hakan adjudicates traffic, [the BOSS orchestrator post](/blog/how-the-boss-orchestrator-works/) explains the routing model.)
