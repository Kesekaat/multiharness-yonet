...
faq:
  ...
  - q: "Can any engine be the BOSS orchestrator?"
    a: "Hakan's engine is swappable from onboarding or a change-engine flow, and OpenCode, Crush, and pi.dev are boss-eligible alongside Claude Code, Antigravity, and Codex. Claude Code is the strongest default because it has the native hook bridge and the deepest hive integration. Copilot CLI is a worker-only engine: its print mode exits per turn, so it can't run the floor."
...
Munder Difflin is built for that second shape. Every hire — and Hakan, the [BOSS orchestrator](/blog/how-the-boss-orchestrator-works/) himself — runs on a pluggable engine, picked from a visual provider picker at hire time. Here's how to lay out the floor.
...
- **OpenCode, Crush, pi.dev** — the BYOK trio (shipped in v0.3.1), wired in via native-plugin, proxy, and hooks bridges respectively. Each works as a worker *and* as Hakan, and each can point at your own keys or a local model.
...
Don't design the org chart up front. Hire Claude Code as Hakan, add one Codex or Copilot desk for a real coding task, and one OpenCode desk on a Groq or Ollama model for something routine. Watch what each engine is actually good at on *your* work, then grow the floor around the evidence.
