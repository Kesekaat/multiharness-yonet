...
- **A git CHANGES rail.** The left side lists every file your agents have touched. Click one and you
  get a **read-only, side-by-side diff against HEAD** — the fastest way to answer "what did Caner
...
it to Hakan in Talk mode). Under the hood the harness drives Copilot's documented non-interactive
print mode:
