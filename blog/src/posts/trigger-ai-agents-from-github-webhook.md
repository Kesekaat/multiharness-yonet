...
- **`POST`** to the endpoint with an `x-md-webhook-secret: <your-secret>` header and a JSON body of
  `{ message, title? }`. On success the message is routed to boss/Hakan, a stamped kanban card is created,
...
message to boss/Hakan** — the orchestrator's inbox — and **files a stamped kanban card**. From there it's
