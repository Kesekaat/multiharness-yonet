...
export interface HiveAgentMeta {
  ...
  cwd: string;
  isManager?: boolean;
  /** Hakan's prep assistant — send-only; enriches prompts and forwards them. */
  isAssistant?: boolean;
}
...
export interface HarnessConfig {
  ...
  /** Realtime Hakan voice loop — true ONLY while a session holds the mic
...
const api = {
   ...
   *  keep running; Hakan is told to stop routing work to them. */
   ...
   *  {} for the whole floor. Backs Realtime Hakan's get_messages. The renderer
   ...
   *  archived agents. Backs Realtime Hakan's get_agent_detail / list_agents. */
  ...
  // ─── Slack integration (Slack message → Hakan's queue) ─────────────────────
  ...
  // Realtime Hakan (voice orchestrator) — MAIN mints a short-lived EPHEMERAL token
  ...
  // human-speakable line Hakan relays; the rest is context for a toast/log.
