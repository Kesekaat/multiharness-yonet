/**
 * Usage telemetry seam (Lane A #6.6 — Seam 1, the LOCKED contract with Bora/#7).
 *
 ...
 *
 * Two invariants every consumer must honor (Bora's 7A.1 spike findings):
 ...
 * "transcript-poll" backend Bora owns and will evolve, then replace with the
 * native-OTel collector. At integration we drop in Bora's module; breaker.ts and
 * the ledger are untouched. The stub's `usd` is the transcript fallback estimate
 * (and inherits the known Sonnet-hardcoded pricing limitation, which Bora fixes
 * in exactly one place — his provider); it is NOT recomputed downstream.
 */
import { readAgentUsage } from './transcript';

/** One cumulative usage snapshot for an agent. The identical row that Bora
 *  emits, Caner (this lane) persists to cost-ledger.jsonl, and Kartal (#4) stores
 ...
 *  normalized form Bora's OTel ingest emits. */
 ...
 * AgentUsageSample. Stands in for Bora's provider until Lane C lands; the
...
export class StubUsageProvider implements UsageProvider {
  ...
  getAgentUsage(agentId: string): AgentUsageSample | null {
    ...
    return {
      ...
      usd: u.estimatedCostUsd // interim fallback estimate; Bora's provider supplies Claude-precomputed usd
    };
  }
}
