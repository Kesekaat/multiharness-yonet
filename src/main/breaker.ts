 ...
 * Bora/#7, whose avatar adapter gives breaker level precedence over hook status).
 *
 * Inputs aggregate three sources:
 *   (a) Bora's usage samples via UsageProvider [Seam 1] — for cost + token velocity;
...
/** Emitted on control:breakerState (Seam 2). One per agent per beat so Bora's
