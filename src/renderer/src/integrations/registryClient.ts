...
//
// CONFORMED to Caner's spec v1 (hive/docs/integrations-spec.md): the types are the
// CANONICAL ones from `@shared/integrations` (Caner's src/shared/integrations.ts),
...
// test). The real path calls window.cth.* (Caner's preload bridge).
//
// ⚠️ The preload bridge is NOT landed yet (Caner owns it — manager relays when it's in).
// Until then this falls back to an in-memory mock so the UI is fully usable in dev.
// The real path is FEATURE-DETECTED: the moment Caner's preload methods appear it
...
//      Caner: expose exactly these (or tell me the names) so the detect matches.
//   2. The catalog served by `integrations:templates` is Caner's `INTEGRATION_TEMPLATES`
//      (the 2 v1 reference templates). Batur's richer src/shared/integrationTemplates.ts
//      is a SEPARATE, currently-unwired file — reconciliation is a manager/Caner/Batur call.
//
...
// The preload bridge Caner exposes (Deliverable 2). Channels are fixed by §6;
...
// Serves Caner's canonical INTEGRATION_TEMPLATES and validates upserts with Caner's
