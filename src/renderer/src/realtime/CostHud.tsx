/**
 * Realtime Hakan — voice session cost HUD (card rt-9, cost-guard).
 *
 * A compact meter for the live voice session's spend. Reads the cost store
 * (costStore.ts), which Kartal's session feeds via resetRealtimeCost() on connect
 * and recordRealtimeUsage() on each usage delta. Self-contained: mount it once
 * near the voice toggle (Hakan's card / fullscreen header) — it renders its own
 ...
 *    so the user (and Hakan, who can read get_cost) knows to wrap up. The actual
